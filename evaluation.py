...
def sample_generate(gen, dst, rows=10, cols=10, seed=0):
    ...
    def make_image(trainer):
        # my code
        os.makedirs(os.path.join(dst, 'gen_imgs'), exist_ok=True)
        x = gen_images(gen, 50000, batchsize=100)
        x = x.transpose(0, 2, 3, 1)
        np.save(os.path.join(dst, 'gen_imgs', 'iter{}.npy'.format(trainer.updater.iteration)), x)
        # finish my code
        np.random.seed(seed)
        n_images = rows * cols
        x = gen_images(gen, n_images, batchsize=n_images)
...
def sample_generate_conditional(gen, dst, rows=10, cols=10, n_classes=1000, seed=0):
    ...
    def make_image(trainer=None):
        ...
        with chainer.using_config('train', False), chainer.using_config('enable_backprop', False):
            x = []
            for c in classes:
                x.append(gen_images_with_condition(gen, c=c, n=5000, batchsize=100))
